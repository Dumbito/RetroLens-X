...
class PortalRenderer:
    """Render the portal content and cinematic VFX into a local ROI."""
    ...
    def _single_buffer(self, cache, shape):
        key = (shape[1], shape[0])
        buffer = cache.get(key)
        if buffer is None or buffer.shape != shape:
            buffer = np.empty(shape, dtype=np.uint8)
    ...
    def _float_buffer(self, cache, shape):
        key = (shape[1], shape[0])
        buffer = cache.get(key)
        if buffer is None or buffer.shape != shape:
