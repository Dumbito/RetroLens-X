...
class PortalVisualConfig:
    ...
    effect_max_width: int = 640
    effect_max_height: int = 480
...
class PortalRenderer:
    ...
    def render(self, frame, dimension, center, width, height, angle, timestamp_ms, intensity=1.0):
        ...
        # Keep the expensive VFX surface bounded. The portal content remains full resolution;
        # the effect surface is downsampled only when the rotated ROI becomes large.
        effect_scale = min(
            1.0,
            float(self.config.effect_max_width) / max(local_w, 1),
            float(self.config.effect_max_height) / max(local_h, 1),
        )
        if effect_scale < 0.999:
            effect_w = max(1, int(round(local_w * effect_scale)))
            effect_h = max(1, int(round(local_h * effect_scale)))
            effect_frame = cv2.resize(local_frame, (effect_w, effect_h), interpolation=cv2.INTER_AREA)
            effect_center = (local_center[0] * effect_scale, local_center[1] * effect_scale)
            effect_width = float(width) * effect_scale
            effect_height = float(height) * effect_scale
            effect_margin = max(1, int(round(margin * effect_scale)))
        else:
            effect_frame = local_frame
            effect_w, effect_h = local_w, local_h
            effect_center = local_center
            effect_width = float(width)
            effect_height = float(height)
            effect_margin = margin

        mask = self._organic_mask((effect_h, effect_w), effect_center, effect_width, effect_height, angle_rad, t)
        if effect_scale < 0.999:
            mask = cv2.resize(mask, (local_w, local_h), interpolation=cv2.INTER_LINEAR)
            effect_mask = self._organic_mask((effect_h, effect_w), effect_center, effect_width, effect_height, angle_rad, t)
        else:
            effect_mask = mask
        ...
        # Draw glow/energy on the bounded effect surface, then upscale once.
        edges = cv2.Canny(effect_mask, 70, 180)
        glow = self._glow_from_edges(edges)
        effect_output = self._buffer(self._canvas_cache, effect_frame.shape)
        effect_output[:] = effect_frame
        self._add_glow(effect_output, glow, intensity)
        energy = self._buffer(self._ring_cache, effect_output.shape)
        energy.fill(0)
        self._draw_rings(energy, effect_center, effect_width, effect_height, angle_rad, t, intensity)
        self._draw_particles(energy, effect_center, effect_width, effect_height, angle_rad, t, intensity)
        effect_blend = self._buffer(self._blend_cache, effect_output.shape)
        cv2.addWeighted(effect_output, 1.0, energy, 0.82 * intensity, 0.0, dst=effect_blend)
        effect_output, effect_blend = effect_blend, effect_output
        effect_rim = self._buffer(self._rim_cache, effect_output.shape)
        effect_rim.fill(0)
        self._draw_rim(effect_rim, effect_center, effect_width, effect_height, angle_rad, t, intensity)
        cv2.addWeighted(effect_output, 1.0, effect_rim, 0.95 * intensity, 0.0, dst=effect_blend)
        effect_output = effect_blend

        if effect_scale < 0.999:
            vfx = cv2.resize(effect_output, (local_w, local_h), interpolation=cv2.INTER_LINEAR)
        else:
            vfx = effect_output

        # Composite content first, then add the VFX result.
        cv2.addWeighted(local_output, 1.0, vfx, 0.35 * intensity, 0.0, dst=local_output)
    ...
    def _organic_mask(self, shape, center, width, height, angle, t):
        ...
        small_mask = self._organic_mask_full((small_h, small_w), small_center, float(width) * scale, float(height) * scale, angle, t)
    ...
    def _draw_rings(self, image, center, width, height, angle, t, intensity=1.0):
        ...
        if arc_count:
            ...
            for i in range(arc_count):
                ...
                axes = (max(1, int(width * (0.42 + i * scale_step))), max(1, int(height * (0.42 + i * scale_step))))
                cv2.ellipse(base, center_point, axes, rotation, start, start + span, (120, 205, 255), 1, cv2.LINE_AA)
